...
|---|---|---|
| `/var/log/os-server.log` | Live backend log (needs `sudo`) | rotates to `/var/log/os-server-<ts>.log` |
...
|---|---|---|
| Ingress | `sensing event received type=motion.activity` in os-server.log | `sudo grep 'sensing event received' /var/log/os-server.log \| tail` |
...
**Fix path:** strengthen MANDATORY directive in `services/server/sensing/delivery/http/handler.go` and `user-emotion-detection/SKILL.md` to explicitly chain to Mood skill.
...
See **[`busy-stuck.md`](./busy-stuck.md)** for full root-cause + diagnostic commands + fix paths. Self-heals after `busyTTL = 5 min` (`services/internal/agent/runtimes/openclaw/service_events.go:29`).
