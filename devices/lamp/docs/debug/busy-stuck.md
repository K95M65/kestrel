# DEV — Lamp Busy-Flag Wedge (Sensing Pipeline Stuck)

Lamp's sensing pipeline goes silent for several minutes: no `[chat.send] >>>`, no new `lifecycle event`, just a stream of `sensing event queued — agent busy ... runId=` (empty runId). OpenClaw side is idle (`active=0 queued=0`). Self-heals after ~5 min via `busyTTL`.

This is **not** a sensing-layer bug — sensing keeps producing events. Lamp is gating them because its `activeTurn` / `IsBusy()` thinks an agent run is still in flight.
...
`busyTTL` is `5 * time.Minute` in `services/internal/agent/runtimes/openclaw/service_events.go:29`. After that the flag forcibly clears and queued sensing events drain (with high-frequency events coalesced + 60s expiry per `drainPendingEvents`).
...
OpenClaw heartbeat turn (target=none) ─────────────────────▶ │ no lifecycle.end SSE
                                                             ▼
                              Lamp busy=true for up to 5 min, drops every sensing chat.send
```

Files:
- Hook source: `services/internal/agent/runtimes/openclaw/hooks/turn-gate/handler.ts`
- Lamp handler: `services/server/openclaw/delivery/sse/handler_api_monitor.go` (`SetBusy`)
- Auto-clear: `services/internal/agent/runtimes/openclaw/service_events.go` (`busyTTL`, `IsBusy`, `drainPendingEvents`)
...
- Lamp journal has many `sensing event queued — agent busy` lines after (2) with no chat.send
...
1. **Hook side (preferred)** — turn-gate skips `/api/openclaw/busy` when OpenClaw turn metadata says `target=none` or `isHeartbeat=true`. Edit `services/internal/agent/runtimes/openclaw/hooks/turn-gate/handler.ts`. This is cheapest and removes the trigger entirely.
