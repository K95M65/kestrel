...
| Chèn instruction + cooldown | `services/domain/voice.go` | `AppendEnrollNudge()` |
| Đường trực tiếp | `services/server/sensing/delivery/http/handler.go` | `PostEvent()` |
| Đường hàng đợi/phát lại | `services/internal/agent/runtimes/openclaw/service.go` | `drainPendingEvents()` |
