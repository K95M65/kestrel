...
- **ISP color corruption** — the same wedged ISP can instead keep delivering **changing** frames whose chroma is garbage: posterized green+magenta, red+magenta, or a broad magenta/deep-magenta palette, with every v4l2 control correct (seen live on the SunplusIT cam right after a close/open cycle). The freeze watchdog cannot see this, so a second watchdog checks the subsampled frame in HSV (throttled to ~1 check/s): a frame is corrupt when either (a) extreme-saturation green covers ≥10% (`_COLOR_GREEN_FRAC`) **and** magenta ≥0.8% (`_COLOR_MAGENTA_FRAC`), (b) red ≥15% (`_COLOR_RED_FRAC`) **and** magenta ≥5% (`_COLOR_RED_MAGENTA_FRAC`), or (c) broad magenta ≥15% (`_COLOR_MAGENTA_DOMINANT_FRAC`) **and** deep magenta (HSV hue 155–169) ≥10% (`_COLOR_DEEP_MAGENTA_FRAC`), at saturation ≥100 (`_COLOR_SAT_MIN`, value ≥60). The companion hue requirements keep a legitimate single-hue red or pink LED spill from triggering recovery. Corruption must be uninterrupted for 30s (`_COLOR_CORRUPT_REOPEN_S`; one clean frame resets) before triggering the same recovery as a freeze. Thresholds were calibrated from live corrupt captures: green 0.19 / magenta 0.012, red 0.23–0.42 / magenta 0.10–0.35, and the blue-LED specimen magenta 0.24 / deep-magenta 0.16. Log line: `Camera color corruption — posterized green/magenta frames for Ns, reopening device`.
