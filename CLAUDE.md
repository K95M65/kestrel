...
1. **Update docs on code change** — When you change code that affects behavior, architecture, or APIs, update **both** the English and Vietnamese docs to match. Keep numbers, flows, endpoints, and states 100% accurate with the code. Platform docs are in `docs/`; lamp-specific docs are in `devices/lamp/docs/`.
   ...
   **Platform docs** (`docs/` + `docs/vi/`):
   ...
   | Claude Code agent backend (`agent_runtime`, internal/agent/runtimes/claudecode, bridge WebSocket, native Telegram channel plugin) | `docs/agentic/claudecode.md` | `docs/vi/agentic/claudecode_vi.md` |
...
Autonomous is an open-source OS for physical AI agents. The Go backend (`services`) provides device onboarding (WiFi, LLM provider, messaging channel setup), OTA updates, and agent gateway integration. The brain is a swappable agentic runtime (OpenClaw, Hermes, or any LLM + skills + memory).
...
- **`services/cmd/os-server/main.go`** — Main HTTP API server (Gin). Handles device setup, network management, LED control, health checks, and agent gateway integration.
- **`services/cmd/bootstrap/main.go`** — OTA bootstrap worker. Periodically checks for and applies updates.
...
- **`internal/`** — Business logic services (ambient, beclient, buddy, device, healthwatch, intent, monitor, network, skills, statusled, vision) plus the agent hub: `internal/agent/` (AgentGateway factory + persona/config/channel/MCP migration) with the swappable backends under `internal/agent/runtimes/{openclaw,hermes,picoclaw,codex,claudecode}`.
...
Config lives in `config/config.json` (path relative to the os-server working dir). Managed by `services/server/config/config.go`. Supports notification channel for config change propagation.
