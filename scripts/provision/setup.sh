...
stage_ota_metadata() {
  ...
  if [ -n "$OTA_SIGNING_PUBLIC_KEY" ]; then
    ...
    cp "$METADATA_ENVELOPE_TMP" "$METADATA_TMP"
    echo "[stage] WARN: OTA signing is not configured; using legacy unsigned metadata"
  fi
  ...
  if [ -z "$WEB_URL" ] || [ -z "$OS_SERVER_URL" ] || [ -z "$BOOTSTRAP_URL" ] || { [ -n "$OTA_SIGNING_PUBLIC_KEY" ] && ! [[ "$WEB_SHA256$OS_SERVER_SHA256$BOOTSTRAP_SHA256" =~ ^[a-fA-F0-9]{192}$ ]]; }; then
...
stage_hal() {
  ...
  if [ -n "$HAL_URL" ]; then
    ...
    [ -z "$HAL_SHA256" ] || echo "$HAL_SHA256  /tmp/hal.zip" | sha256sum -c - >/dev/null || { echo "[stage] ERROR: HAL SHA-256 mismatch"; return 1; }
...
EOF
  ...
  SKILLS_CATALOG="asl-teacher audio body-play camera cameraman claude-buddy computer-use connectors dance display emotion face-enroll focus-coach greeter guard habit hive home-assistant input-branching kitchen led-control mood morning-brief music music-suggestion news photobooth pomodoro radio remember robot-apps scene sensing sensing-track servo-control servo-tracking skill-creator speaker-recognizer spotify stories user-emotion-detection voice weather wellbeing"
  ...
  skill_caps() {
    case "$1" in
      asl-teacher) echo "motion audio" ;;
      audio) echo audio ;;
      body-play) echo motion ;;
      camera) echo vision ;;
      cameraman) echo "motion vision" ;;
      claude-buddy) echo audio ;;
      computer-use) echo companion ;;
      dance) echo "motion audio" ;;
      display) echo display ;;
      emotion) echo expression ;;
      face-enroll) echo presence ;;
      focus-coach) echo vision ;;
      greeter) echo presence ;;
      guard) echo presence ;;
      led-control) echo light ;;
      music) echo media ;;
      news) echo audio ;;
      photobooth) echo "vision audio" ;;
      pomodoro) echo audio ;;
      radio) echo audio ;;
      scene) echo light ;;
      sensing) echo sensing ;;
      sensing-track) echo sensing ;;
      servo-control) echo motion ;;
      servo-tracking) echo motion ;;
      speaker-recognizer) echo audio ;;
      spotify) echo companion ;;
      stories) echo audio ;;
...
server {
  ...
  # Autonomous Buddy (macOS companion) persistent WebSocket. Same Upgrade + long-
...
if [ "$APP" = "rollback" ]; then
  ...
  if [ -n "$BLOCKED_VERSION" ] && [ -f "$BOOTSTRAP_JSON" ]; then
    ...
    echo "WARN: could not determine rolled-back version; lower min_version before the next OTA poll"
  fi
  install -D -m 0755 "$BACKUP" "$DEST"
  systemctl restart "$UNIT"
  # Reload bootstrap's in-memory rollback block. It is harmless when bootstrap
  # itself was the target (the preceding restart has already replaced it).
...
stage_devices() {
  ...
  [ -z "$DEVICES_SHA256" ] || echo "$DEVICES_SHA256  /tmp/device.zip" | sha256sum -c - >/dev/null \
