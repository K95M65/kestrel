...
# 3a''. nginx /api/buzz/ws: hive WebSocket (same Upgrade as buddy).
python3 - "$NGINX_CONF" <<'PYEOF'
import sys
path = sys.argv[1]
with open(path) as f:
...
if "location = /api/buzz/ws" in content:
    print("[patch] nginx /api/buzz/ws: already present, skipping")
    sys.exit(0)

old = "  location /api/ {"
new = """  # Kestrel hive (Buzz) WebSocket — must come before generic /api/.
...
if old not in content:
    print("[patch] nginx /api/buzz/ws: /api/ block not found, may need manual check")
    sys.exit(0)

content = content.replace(old, new, 1)
with open(path, "w") as f:
    f.write(content)
print("[patch] nginx /api/buzz/ws: WebSocket upgrade block added")
PYEOF
