...
#    agent-runtimes/hermes/constants.go BaseURL.
set -euo pipefail
...
# config.yaml is owned by the presync hook (agent-runtimes/hermes/presync.sh), not this
# installer — see the presync section below.

# API_SERVER_KEY MUST equal agent-runtimes/hermes/constants.go APIKey — os-server sends
...
# this installer runs (see agent-runtimes/hermes/presync.sh + system/device materialize).
