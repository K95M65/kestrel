...
- [x] **Bridge (Go)** `agent-runtimes/codex/gatewayd/` — WS server (gorilla), auth 4401, single-client
...
- [x] Go package `agent-runtimes/codex/` — full AgentGateway: translator.go (codex exec JSONL → WSEvent),
...
- [x] Slack inbound, HTTP mode (2026-07-07): CodexService implements domain.SlackBridge
      (slack.go, modeled on agent-runtimes/hermes/slack.go), so the existing bff-proxy → MQTT
