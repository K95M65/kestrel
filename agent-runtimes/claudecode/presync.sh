...
# (agent-runtimes/claudecode/gatewayd — Go port of the former bridge.py), so a plain
...
# writes.
#
# This file is EMBEDDED IN os-server (agent-runtimes/claudecode/presync.sh) and
...
# connector entries there — agent-runtimes/claudecode/mcp.go) without the interactive
# approval prompt.
...
# Two auth modes, decided by the claude login flow (agent-runtimes/claudecode/login.go):
#
