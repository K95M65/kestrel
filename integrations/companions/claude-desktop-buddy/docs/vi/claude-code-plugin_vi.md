...
Các endpoint push (`/claude-code/notify`, `/usage`, `/approval-request`) được bảo
vệ bằng **mật khẩu admin của thiết bị** — chính mật khẩu dùng để đăng nhập web UI
của thiết bị — gửi dưới dạng `Authorization: Bearer <password>`. Plugin lưu mật
khẩu này theo từng thiết bị trong `~/.config/claude-code-buddy.json` (trường
`password`, quyền `0600`) và tự động đính kèm header. Mật khẩu thiếu hoặc cũ sẽ trả
về **HTTP 401**; khi đó plugin hỏi người dùng mật khẩu admin hiện tại rồi cập nhật
mục đã lưu. Khám phá (`GET /health`) vẫn không cần xác thực để thiết bị vẫn tìm
thấy được trước khi đặt mật khẩu. Các ví dụ `curl` bên dưới thêm
...
BLE/cấp quyền: `GET /status`, `POST /claude-desktop/approve`,
`POST /claude-desktop/deny` (tất cả đều **chỉ loopback** — agent trên thiết bị gọi
chúng), cộng với `GET /health` không cần xác thực. Xem
[`architecture_vi.md`](architecture_vi.md#http-api-security).
...
|----------|------|---------|
| `POST /claude-code/approval-request` | `{id, tool, input, hint?}` | Long-poll; trả về `{"decision":"allow"｜"deny"｜"timeout"}`. **Bearer token mật khẩu admin (`401` nếu không)** — đây là lời gọi LAN từ plugin. `400` nếu thiếu `id`. |
...
| `GET /claude-code/pending` | — | `{"pending":[{id,tool,hint}]}` — những gì đang chờ một câu trả lời bằng giọng nói. **Chỉ loopback (`403` nếu không).** |
...
- **Plugin** (`~/.config/claude-code-buddy.json`) — cờ mới `approval_enabled`
  ...
  Mỗi mục thiết bị cũng mang theo `password` (mật khẩu admin của thiết bị) dùng để
  xác thực các push qua LAN.
- **Daemon** (`config/buddy.json`) — `code_approval_ttl_sec` (mặc định **55**): một
  yêu cầu long-poll trong bao lâu trước khi trả về `"timeout"`; `os_config_path`
  (mặc định là một `config.json` cùng cấp với `buddy.json`): nơi daemon đọc
  `admin_password_hash` để xác minh Bearer token.
...
- **Cấu hình (Mac)** — lưu tại `~/.config/claude-code-buddy.json`. Nó ghi địa chỉ
  thiết bị, **mật khẩu admin** của thiết bị (dùng để xác thực các push), và các tùy
  chọn notify/usage/sound của plugin. Plugin tự sửa file này khi bạn yêu cầu bằng
  ngôn ngữ tự nhiên ("mute my device", "warn me earlier"); không cần khởi động lại.
