...
final class PairingWindowController: NSWindowController {
    ...
    private func buildLayout() {
        ...
        let hostLabel = NSTextField(labelWithString: "Device host")
    ...
    @objc private func submit() {
        ...
        guard code.count == 6, code.allSatisfy({ $0.isNumber }) else {
            statusLabel.stringValue = "Enter the 6-digit code shown by your device's web UI."
            return
        }
        ...
        Task { @MainActor in
            do {
                let record = try await manager.pair(deviceHost: host, code: code)
                statusLabel.textColor = .systemGreen
                statusLabel.stringValue = "Paired with \(record.deviceHost)"
                onSuccess?(record)
