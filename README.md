...
Everything outside `hal/` is Apache-2.0. `hal/` is GPL-3.0 — a handful of modules, the `follower/` package and the teleop recordings are inherited from [LeLamp Runtime](https://github.com/humancomputerlab/lelamp_runtime) ([`hal/UPSTREAM.md`](hal/UPSTREAM.md) lists them); the rest of `hal/` is ours, kept GPL by choice so the tree has one license per top-level folder. A driver you commit there is GPL, so a closed vendor SDK wraps out of process. A robot running this also carries other people's work — the [LeLamp Runtime](https://github.com/humancomputerlab/lelamp_runtime) our motion code grew out of, Pollen's `reachy_mini` SDK, TEN-VAD and Silero for hearing, YOLOv8 for tracking (AGPL-3.0 — read it before you ship), and the brains we install but do not ship. All of it, with what we copied verbatim: [`CREDITS.md`](CREDITS.md). Security issues: [`SECURITY.md`](SECURITY.md).
