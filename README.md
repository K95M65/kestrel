...
We host three things — the skill store, the AI gateway, the update feed. Nothing else phones home. Every robot skill is in this repo; the store also carries workflow skills that need no hardware ([`docs/hosted.md`](docs/hosted.md)). We don't run an app store: **the app store on every robot is the Hugging Face Hub** — push a Space tagged `autonomous-os-plugin` and it appears under Settings → Plugins → Browse on every robot.
