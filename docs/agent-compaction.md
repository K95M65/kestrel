...
>
> This doc is the reference linked from the **📋 Summary** button in Flow Monitor (modal: `services/web/src/pages/monitor/FlowSection/CompactionModal.tsx`).
...
| File | Role |
|---|---|
| `services/server/openclaw/delivery/sse/handler_api_compaction.go` | HTTP handler: reads `sessions.json`, scans session `.jsonl` for newest `type:"compaction"`. |
| `services/server/openclaw/delivery/sse/handler_events.go` | OS-server-side RPC trigger (auto-compact when `TotalTokens > 80_000`, TTS notice, 2-min cooldown). |
| `services/internal/agent/runtimes/openclaw/service_chat.go` | `CompactSession(sessionKey)` — the `sessions.compact` RPC sender. |
| `services/domain/agent.go` | `AgentGateway.CompactSession` interface. |
| `services/web/src/pages/monitor/FlowSection/CompactionModal.tsx` | UI modal — shows timestamp, summary chars, session file, full summary text; links back to this doc. |
