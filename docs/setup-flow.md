...
1. **Phase poll** — polls `GET /api/device/setup/status` against the AP IP while
   ...
   `internal/device/service.go`.) A wall-clock watchdog flips an `apLost` flag
...
| File | Role |
|------|------|
| `services/internal/device/service.go` | Setup orchestration + early LAN-IP capture goroutine |
| `services/web/src/lib/setupBridge.ts` | Parent-window event bridge (postMessage) |
| `services/web/src/pages/Setup.tsx` | Setup wizard UI + bridge emit call sites + IP-first copy link |
| `services/web/src/hooks/setup/useSetupStatusPolling.ts` | AP→STA auto-redirect: phase poll + LAN-IP probe + mDNS probe |
| `services/web/src/hooks/setup/useWifiConnected.ts` | Post-reload Wi-Fi-done detection from live device state (`check-internet` + `network/current`) |
| `services/internal/network/service.go` | WiFi connect, AP mode, `CurrentNetwork()` (associated SSID) |
| `services/server/device/delivery/http/handler.go` | HTTP setup handler (async goroutine) |
