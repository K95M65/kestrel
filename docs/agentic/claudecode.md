...
| Bridge (systemd `claudecode.service`) | `os-server claudecode-gatewayd` subcommand (compiled into `/usr/local/bin/os-server`; code `agent-runtimes/claudecode/gatewayd/`) |
...
(`agent-runtimes/claudecode/gatewayd/`, structurally mirroring the codex gatewayd —
...
run inside os-server, mirroring `agent-runtimes/codex` 1:1. Claude Code's native
...
- **Slack is DEVICE-OWNED** (`slack.go` + `slack_sender.go`, a mirror of
  `agent-runtimes/codex/slack.go`): Claude Code has no slack channel plugin ("Claude
...
of `llm_api_key`. The flow (`agent-runtimes/claudecode/login.go`, the
