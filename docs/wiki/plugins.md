...
**Robot apps** are what Device → Plugins lists. Trusted from this repo: dance, emotions, cameraman, phrase teacher, radio, photobooth. Install by name. They run as their own process (systemd) and talk to HAL. Start / stop from that list, or in Talk: “put the radio on”, “stop cameraman” (`[HW:/plugin/start]`). Cameraman and photobooth use the camera until Stop; kids profile keeps those off. Radio uses the speaker until Stop.
