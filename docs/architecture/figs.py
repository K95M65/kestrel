"""Tokens and shapes for the Autonomous OS figures — the register Anthropic's
engineering posts use. Copied verbatim from autonomous-grid/docs/figs.py so every
Autonomous repo draws from one token set; the standard itself lives in
autonomous-org/knowledge/diagram-style.md.
...
class Fig:
    ...
    def box(self, cx, cy, label, kind="green", w=None, dashed=False):
        ...
        self.parts.append(
            ...
            f'fill="{fill}" stroke="{line}" stroke-width="{BORDER}"{dash}/>')
        self.parts.append(
    ...
    def term(self, cx, cy, label, w=None, dashed=False):
        """A terminal — circle when the label is short, stadium when it is not.

        `dashed` follows the house rule for edges: dashed means optional or not
        yet — a body whose contract exists but whose driver does not.
        """
        ...
        self.parts.append(
            ...
            f'fill="{CORAL_FILL}" stroke="{CORAL_LINE}" stroke-width="{BORDER}"{dash}/>')
        self.parts.append(
