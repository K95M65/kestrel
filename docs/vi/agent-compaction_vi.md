...
>
> Doc này là reference link từ nút **📋 Summary** ở Flow Monitor (modal: `services/web/src/pages/monitor/FlowSection/CompactionModal.tsx`).
...
| **OS server RPC** (`services/server/openclaw/delivery/sse/handler_events.go:380-406`) | OS server thấy `u.TotalTokens > 80_000` trên lifecycle event, gọi `agentGateway.CompactSession(sessionKey)` | TTS nói *"Hold on, tidying up a bit."*; cooldown 2 phút qua `h.compacting` atomic | chưa rõ — cần verify từ source OpenClaw |
...
| File | Vai trò |
|---|---|
| `services/server/openclaw/delivery/sse/handler_api_compaction.go` | HTTP handler: đọc `sessions.json`, scan session `.jsonl` tìm `type:"compaction"` mới nhất. |
| `services/server/openclaw/delivery/sse/handler_events.go` | RPC trigger phía OS server (auto-compact khi `TotalTokens > 80_000`, TTS notice, cooldown 2 phút). |
| `services/internal/agent/runtimes/openclaw/service_chat.go` | `CompactSession(sessionKey)` — sender của `sessions.compact` RPC. |
| `services/domain/agent.go` | Interface `AgentGateway.CompactSession`. |
| `services/web/src/pages/monitor/FlowSection/CompactionModal.tsx` | UI modal — show timestamp, summary chars, session file, toàn văn summary; link về doc này. |
