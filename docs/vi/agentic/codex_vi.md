...
- **`picoclaw`**: client WebSocket bền tới runtime PicoClaw cục bộ. Xem `docs/agentic/picoclaw.md` + `agent-runtimes/picoclaw`.
- **`codex`**: **OpenAI Codex CLI** làm bộ não agent của thiết bị, sau một WS bridge cục bộ. Tài liệu này. Code: `agent-runtimes/codex/`.

> Code là nguồn chân lý. Tài liệu này mô tả `agent-runtimes/codex/` đúng như đã triển
...
bridge được **compile thẳng vào binary os-server** (`agent-runtimes/codex/gatewayd`,
...
Hằng số kết nối (`agent-runtimes/codex/constants.go`, không có config theo máy):
...
os-server tự chạy receive loop inbound: `agent-runtimes/codex/telegram_poll.go`, một
...
của hermes, `agent-runtimes/hermes/slack.go`): proxy công khai bff-campaign-service
...
(`agent-runtimes/codex/slack.go`), nên event chỉ được route về đây **khi codex là
...
(`agent-runtimes/codex/discord.go`). Như loop telegram, phiên được khởi động từ
...
Mirror `agent-runtimes/claudecode/telegram_coding.go` 1:1 — một chat Telegram có thể
...
tái hiện **native bằng Go** (`agent-runtimes/codex/emotion_ack.go`, mirror
