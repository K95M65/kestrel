...
Date: 2026-05-16  
Repo: `lamp`  
Scope: Lamp Go server only (`services/server`, `services/internal`, `services/domain`, nginx `/api/` wiring).  
...
In `services/server/server.go`, the router uses:
...
`services/server/server.go` creates `api := r.Group("api")` and then registers sensitive routes directly. Examples:
...
#### File: `services/server/server.go`
...
Route registration in `services/server/server.go`:
...
In `services/server/server.go`, remove:
...
Response struct includes secrets in `services/domain/device.go`:
...
#### File: `services/domain/device.go`
...
#### File: `services/internal/device/service.go`
...
#### File: `services/server/device/delivery/http/handler.go`
...
Do not let config updates restart `hal` unlimited times. Add debounce:
...
Routes in `services/server/server.go`:
...
`services/server/config/config.go` has only:
...
`services/bootstrap/bootstrap.go`:
...
File: `services/server/server.go` or new file `services/server/security.go`.
