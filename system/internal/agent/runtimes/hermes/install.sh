...
#    internal/agent/runtimes/hermes/constants.go BaseURL.
set -euo pipefail
...
# config.yaml is owned by the presync hook (internal/agent/runtimes/hermes/presync.sh), not this
# installer — see the presync section below.

# API_SERVER_KEY MUST equal internal/agent/runtimes/hermes/constants.go APIKey — os-server sends
...
# this installer runs (see internal/agent/runtimes/hermes/presync.sh + internal/device materialize).
