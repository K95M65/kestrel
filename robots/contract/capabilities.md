...
- **Expression / output (OUT).** Driven *down* from agent → os-server → HAL → body:
  ...
  - **`lifelike`** is the personality opt-in: the body behaves like a living creature when
    ...
    `lifelike` stays a quiet tool when idle. See `robots/lamp/docs/sensing-behavior.md`
...
`policy` identifier and a `task`; `POST /policy/stop` cancels an accepted run.
The initial implementation is explicitly
