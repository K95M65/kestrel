...
`robots/contract/cts/` is the automated enforcer of [`robots/contract/COMPATIBILITY.md`](../COMPATIBILITY.md).
...
```bash
make cts          # or: python3 -m unittest discover -s robots/contract/cts -v
```

Validates every device under `robots/` against the MUST rules it can read off `ROBOT.md`:
...
scp robots/contract/cts/test_runtime.py <user>@<device-ip>:/tmp/
