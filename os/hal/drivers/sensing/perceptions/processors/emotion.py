...
# Per-trigger debug capture — kept local to this module (read straight from the
# environment) so the feature is self-contained. Every emotion-recognizer
# trigger writes its own timestamped folder (input crop + annotated frame +
# result.json) under EMOTION_LOG_DIR, so misclassifications can be eyeballed
# against ground truth.
EMOTION_DEBUG_LOG_ENABLED = (
    os.environ.get("HAL_EMOTION_DEBUG_LOG_ENABLED", "true").lower() == "true"
)
EMOTION_LOG_DIR = os.environ.get(
    "HAL_EMOTION_LOG_DIR", "/opt/hal/drivers/sensing/perceptions/processors/emotion-logs"
)
...
class EmotionDebugLogger:
    ...
    under ``EMOTION_LOG_DIR``, so each prediction can be eyeballed against what
...
class EmotionPerception(Perception[FaceDetectionData]):
    ...
    def __init__(
        ...
        self._debug: EmotionDebugLogger = EmotionDebugLogger(
            root_dir=EMOTION_LOG_DIR,
            enabled=EMOTION_DEBUG_LOG_ENABLED,
            max_triggers=EMOTION_LOG_MAX_TRIGGERS,
        )
        if EMOTION_DEBUG_LOG_ENABLED:
            logger.info("[activity.emotion] debug logging → %s", EMOTION_LOG_DIR)
