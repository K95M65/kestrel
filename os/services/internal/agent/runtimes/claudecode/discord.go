...
// Device-owned Discord inbound for the Claude Code runtime.
//
...
// session — mirroring internal/agent/runtimes/codex/discord.go 1:1. Claude Code DOES ship a
// native discord channel plugin, but it shares the telegram plugin's failure
// modes (bun black box: no journal logs, silent allowlist drops, silent death
// on bridge-restart races), so it is deliberately not used — presync no
// longer launches any channel plugin. Like the telegram loop
// (telegram_poll.go), this is started from StartWS and lives inside this
// service's lifecycle — it runs ONLY while claudecode is the active runtime,
// so it can never fight another runtime's bot session for the same token.
//
...
func (s *ClaudeCodeService) startDiscordBot(ctx context.Context) {
	slog.Info("discord bot loop started", "component", "claudecode")
	for {
		select {
		case <-ctx.Done():
			return
		...
		if err != nil {
			...
			slog.Warn("discord session open failed", "component", "claudecode", "error", err)
		...
		if err := sess.Close(); err != nil {
			slog.Debug("discord session close failed", "component", "claudecode", "error", err)
		}
		return
	}
}
...
func (s *ClaudeCodeService) openDiscordSession(ctx context.Context, token string) (*discordgo.Session, error) {
	...
	slog.Info("discord session open", "component", "claudecode", "botUserId", botID)
	return sess, nil
}
...
// consistent value.
func (s *ClaudeCodeService) setDiscordSession(sess *discordgo.Session) {
	s.discordMu.Lock()
	s.discordSession = sess
	s.discordMu.Unlock()
}

func (s *ClaudeCodeService) getDiscordSession() *discordgo.Session {
	s.discordMu.Lock()
...
func (s *ClaudeCodeService) handleDiscordMessage(ctx context.Context, sess *discordgo.Session, m *discordgo.MessageCreate) {
	if m.Author == nil {
		return
	}
	botUserID := ""
	...
	if !ok {
		slog.Debug("discord message skipped",
			"component", "claudecode", "authorId", m.Author.ID, "guildId", m.GuildID)
		return
	}
...
func (s *ClaudeCodeService) injectDiscordTurn(ctx context.Context, text, channelID string) {
	for s.IsBusy() {
	...
	if err := send(text, reqID, runID); err != nil {
		...
		slog.Error("discord turn injection failed",
			"component", "claudecode", "runID", runID, "channelID", channelID, "error", err)
		return
	}
...
func (s *ClaudeCodeService) discordTypingKeeper(ctx context.Context, channelID, runID string) {
...
func (s *ClaudeCodeService) sendDiscordTyping(channelID string) {
	sess := s.getDiscordSession()
	if sess == nil {
		return
	}
	if err := sess.ChannelTyping(channelID); err != nil {
		slog.Debug("discord ChannelTyping failed", "component", "claudecode", "error", err)
	}
}
...
func (s *ClaudeCodeService) finishDiscordTurn(channelID, reply string) {
	if reply == "" {
		return
	}
	...
	for _, chunk := range chunkDiscordMessage(reply, discordMaxMessageLen) {
		if err := send(channelID, chunk); err != nil {
			slog.Error("discord reply send failed",
				"component", "claudecode", "channelID", channelID, "error", err)
			return
		}
	}
	slog.Info("discord reply posted", "component", "claudecode", "channelID", channelID)
}
