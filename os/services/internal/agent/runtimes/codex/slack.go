...
// Device-owned Slack inbound (HTTP mode) for the Codex runtime, modeled on
// internal/agent/runtimes/hermes/slack.go.
//
// The public bff-campaign-service proxy receives Slack Events API deliveries
// and fans them out over MQTT to the device's slack_event handler
...
// CodexService implements that bridge, so while codex is the active runtime a
// forwarded Slack message becomes a regular codex turn: the run is tracked in
// slackRuns (reply routing) and marked silent (no TTS), and emitFinal posts
// the reply back to the originating channel/thread via chat.postMessage
// (translator.go), mirroring the telegram pattern in telegram_poll.go.
//
// Intentional differences from hermes: NO progressive streaming
// (chat.startStream/appendStream — codex exec emits the reply whole, so the
// final text is posted once) and NO assistant "…is typing" status (that
// affordance belongs to the streaming UX). The eyes ack reaction is mirrored:
// added when the turn is injected, cleared when the reply (or error) lands.

// Compile-time check: *CodexService implements domain.SlackBridge, which is
// what the slack_event MQTT handler type-asserts to route inbound events here.
var _ domain.SlackBridge = (*CodexService)(nil)
...
// the reply in the same channel/thread and clear the ack reaction. Mirrors
// telegramRuns (runID → chat id) with the extra thread/message coordinates
// Slack needs.
...
// message as a codex turn. Injection happens in a goroutine (the agent may be
// busy and codex correlates turns by a single in-flight runID, so the turn
// waits for idle like the telegram path) — handled == true therefore means
// "accepted for injection", and the MQTT handler acks immediately.
func (s *CodexService) HandleInboundSlack(in domain.SlackInbound) (string, bool, error) {
	challenge, msg, err := parseSlackInbound(in.Body, s.config.SlackUserID)
	if err != nil {
		return "", false, err
	}
	if challenge != "" {
		slog.Info("slack: url_verification challenge", "component", "codex")
	...
	slog.Info("slack: inbound accepted for codex turn", "component", "codex", "channel", msg.channel)
	return "", true, nil
}
...
// Mirrors injectTelegramTurn.
func (s *CodexService) injectSlackTurn(text string, origin slackRun) {
	deadline := time.Now().Add(slackBusyWaitMax)
	for s.IsBusy() {
		if time.Now().After(deadline) {
			slog.Warn("slack turn dropped (agent busy past cap)",
				"component", "codex", "channel", origin.channel, "cap", slackBusyWaitMax)
			return
		}
	...
	if err := send(text, reqID, runID); err != nil {
		...
		slog.Error("slack turn injection failed",
			"component", "codex", "runID", runID, "channel", origin.channel, "error", err)
		return
	}
	// Acknowledge receipt with an eyes reaction (best-effort, mirrors hermes /
	// openclaw's slack plugin); finishSlackTurn clears it when the reply lands.
	if origin.messageTS != "" {
		if err := s.setSlackReaction(true, origin.channel, origin.messageTS, slackAckReaction); err != nil {
			slog.Debug("slack: ack reaction failed (non-fatal)", "component", "codex", "err", err)
		}
	}
}
...
// rides the silent-run marker (set at injection), like the telegram path.
func (s *CodexService) IsSlackOriginRun(runID string) bool {
	return s.hasSlackRun(runID)
}

// StreamSlackDelta implements domain.SlackBridge — intentionally a no-op:
// codex exec emits the reply whole (no delta stream), so there is nothing to
// stream progressively; the full reply is posted once by emitFinal.
func (s *CodexService) StreamSlackDelta(string, string) {}
...
// non-Slack runs.
func (s *CodexService) DeliverSlackReply(runID, text string) error {
...
func (s *CodexService) finishSlackTurn(o slackRun, reply string) {
	if o.messageTS != "" {
		if err := s.setSlackReaction(false, o.channel, o.messageTS, slackAckReaction); err != nil {
			slog.Debug("slack: clear ack reaction failed (non-fatal)", "component", "codex", "err", err)
		}
	}
	if reply == "" {
		return
	...
	if err := s.postSlackMessage(o.channel, o.threadTS, reply); err != nil {
		slog.Error("slack reply send failed", "component", "codex", "channel", o.channel, "error", err)
		return
	}
	slog.Info("slack reply posted", "component", "codex", "channel", o.channel, "threaded", o.threadTS != "")
}
