...
func (s *CodexService) StartHALVoice(deepgramKey, llmKey, sttKey, ttsKey, llmBaseURL, sttBaseURL, ttsBaseURL, ttsVoice, ttsInstructions, ttsProvider string) error {
	if deepgramKey == "" {
		return nil
	}
	...
	slog.Info("HAL voice pipeline started", "component", "codex")
...
func (s *CodexService) SetVolume(pct int) error {
	if err := hal.SetVolume(pct); err != nil {
		return err
	}
	slog.Info("speaker volume set", "component", "codex", "pct", pct)
	return nil
}

func (s *CodexService) StopTTS() error {
	if err := hal.StopTTS(); err != nil {
		return err
	}
	if err := hal.StopAudio(); err != nil {
		slog.Warn("stop audio failed", "component", "codex", "error", err)
	}
	slog.Info("speaker stopped (TTS + music)", "component", "codex")
	return nil
}

func (s *CodexService) SendToHALTTS(text string) error {
	...
	slog.Info("TTS sent", "component", "codex", "text", truncRunes(text, 80))
	s.monitorBus.Push(domain.MonitorEvent{Type: "tts", Summary: text})
	return nil
}

func (s *CodexService) SendToHALTTSQueue(text string) error {
	text = stripForTTS(text)
	if text == "" {
		return nil
	}
	...
	slog.Info("TTS queued", "component", "codex", "text", truncRunes(text, 80))
	s.monitorBus.Push(domain.MonitorEvent{Type: "tts", Summary: text})
	return nil
}
