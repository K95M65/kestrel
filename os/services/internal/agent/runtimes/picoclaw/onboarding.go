...
// Identical template to internal/agent/runtimes/openclaw/resources/KNOWLEDGE.md.
//
//go:embed resources/KNOWLEDGE.md
var knowledgeFS embed.FS

// Onboarding (PicoClaw). Mirrors internal/agent/runtimes/openclaw/onboarding.go, but trimmed to
// what PicoClaw actually owns on-device:
//
//   - The PicoClaw runtime is installed + provisioned out-of-process by
//     internal/agent/runtimes/picoclaw/install.sh + presync.sh (binary, `picoclaw onboard`,
...
const (
	...
	// Derived from internal/agent/runtimes/openclaw/onboarding.go agentsMDBlock with OpenClaw-only
...
func (s *PicoclawService) EnsureOnboarding() error {
	...
	// Hermes observer hook (internal/agent/runtimes/hermes/hooks.go). Best-effort: a hook failure
...
// internal/agent/runtimes/openclaw/onboarding.go (package-private there).
...
// Copied from internal/agent/runtimes/openclaw/onboarding.go (package-private there).
