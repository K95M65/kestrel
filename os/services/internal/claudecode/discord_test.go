...
// seams, mirroring slack_test.go and the codex discord tests.
...
func TestDiscordRunMapRoundTrip(t *testing.T) {
	s := &ClaudeCodeService{discordRuns: make(map[string]string)}
...
// the send seam. The final text rides claudeEvent.Result (Claude Code's
// result event), unlike codex' accumulated assistantParts.
func TestDiscordReplyRoutingEmitFinal(t *testing.T) {
	...
	s.emitFinal(claudeEvent{Result: "[HW:/led/set:{\"r\":1}] Done! [laugh]"}, func(domain.WSEvent) {})
...
func TestDiscordReplyChunkedOnEmitFinal(t *testing.T) {
	...
	s.emitFinal(claudeEvent{Result: long}, func(domain.WSEvent) {})
