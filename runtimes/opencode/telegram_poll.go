...
func (s *OpenCodeService) startTelegramPoll(ctx context.Context) {
	...
	slog.Info("telegram poll loop started", "component", "opencode", "offset", offset)
	for {
		select {
		case <-ctx.Done():
			return
		...
		if err != nil {
			...
			slog.Warn("telegram getUpdates failed", "component", "opencode", "error", err)
...
func (s *OpenCodeService) handleTelegramUpdate(ctx context.Context, u tgUpdate, allowedUser string) {
	...
	if msg == nil || strings.TrimSpace(msg.Text) == "" {
		slog.Debug("telegram update skipped (no text message)", "component", "opencode", "updateId", u.UpdateID)
		return
	}
	if msg.Chat.Type != "private" {
		slog.Debug("telegram update skipped (not a private chat)",
			"component", "opencode", "updateId", u.UpdateID, "chatType", msg.Chat.Type)
		return
	}
	fromID := ""
	if msg.From != nil {
	...
	if allowedUser == "" || fromID != allowedUser {
		slog.Debug("telegram update skipped (sender not allowlisted)",
			"component", "opencode", "updateId", u.UpdateID, "fromId", fromID)
		return
	}
...
func (s *OpenCodeService) sendTelegramTyping(ctx context.Context, chatID string) {
	token := s.config.TelegramBotToken
	if token == "" {
		return
	}
	...
	if err != nil {
		slog.Debug("telegram sendChatAction failed", "component", "opencode", "error", err)
		return
	}
	_ = resp.Body.Close()
}
...
func (s *OpenCodeService) fetchTelegramUpdates(ctx context.Context, client *http.Client, token string, offset int64) ([]tgUpdate, error) {
	base := s.telegramAPIBase
	if base == "" {
		base = telegramAPIBaseDefault
	}
...
func (s *OpenCodeService) telegramOffsetFilePath() string {
...
func (s *OpenCodeService) loadTelegramOffset() int64 {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Warn("telegram offset read failed", "component", "opencode", "error", err)
		}
		return 0
	}
	var st telegramOffsetState
	if err := json.Unmarshal(data, &st); err != nil {
		slog.Warn("telegram offset parse failed", "component", "opencode", "error", err)
		return 0
	}
	return st.Offset
}
...
func (s *OpenCodeService) saveTelegramOffset(offset int64) {
	path := s.telegramOffsetFilePath()
	data, err := json.Marshal(telegramOffsetState{Offset: offset})
	if err != nil {
		slog.Warn("telegram offset marshal failed", "component", "opencode", "error", err)
		return
	}
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		slog.Warn("telegram offset dir create failed", "component", "opencode", "error", err)
		return
	}
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, data, 0o600); err != nil {
		slog.Warn("telegram offset write failed", "component", "opencode", "error", err)
		return
	}
	if err := os.Rename(tmp, path); err != nil {
		slog.Warn("telegram offset rename failed", "component", "opencode", "error", err)
	}
}
