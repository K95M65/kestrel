...
// Device-owned Slack inbound (HTTP mode) for the OpenCode runtime, modeled on
...
// OpenCodeService implements that bridge, so while opencode is the active runtime a
// forwarded Slack message becomes a regular opencode turn: the run is tracked in
...
// (chat.startStream/appendStream — opencode run emits the reply whole, so the
...
// message as a opencode turn. Injection happens in a goroutine (the agent may be
// busy and opencode correlates turns by a single in-flight runID, so the turn
