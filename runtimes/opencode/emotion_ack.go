...
// emotion-acknowledge parity for OpenCode.
//
...
// so the body shows it is working before the reply lands. OpenCode ships no
// ~/.opencode/hooks loader (the handler.ts copied in by `claw migrate` is never
// executed under OpenCode), so we reproduce the same behavior natively here,
...
func (s *OpenCodeService) fireAckEmotion(runID, message string) {
	if !s.ackHookEnabled {
		return
	}
	...
	safego.Go("opencode-ack-emotion", func() {
		if err := hal.SetEmotion(ackEmotionName, ackEmotionIntensity); err != nil {
			slog.Debug("ack emotion post failed", "component", "opencode", "error", err)
		}
	})
}
