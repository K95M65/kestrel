...
// SetSessionKey stores the session id. OpenCode assigns it on its first inbound
// frame (translateFrame captures it), so the read loop is the usual caller.
func (s *OpenCodeService) SetSessionKey(key string) {
	s.sessionUUID.Store(key)
	slog.Info("session key stored", "component", "opencode", "key", key)
	flow.Log("session_key_acquired", map[string]any{"key_len": len(key)})
}

// GetSessionKey returns the OpenCode session id or "".
func (s *OpenCodeService) GetSessionKey() string {
	v, _ := s.sessionUUID.Load().(string)
	return v
}

func (s *OpenCodeService) MarkGuardRun(runID string, snapshotPath string) {
	s.guardRunsMu.Lock()
	s.guardRuns[runID] = snapshotPath
	s.guardRunsMu.Unlock()
	slog.Info("guard run marked", "component", "opencode", "runID", runID, "snapshot", snapshotPath)
}

func (s *OpenCodeService) ConsumeGuardRun(runID string) (string, bool) {
	s.guardRunsMu.Lock()
...
func (s *OpenCodeService) MarkPoseBucketRun(runID string, bucketID string, worstFilenames []string) {
	if runID == "" || bucketID == "" {
		return
	}
	...
	slog.Info("pose bucket run marked",
		"component", "opencode", "runID", runID, "bucket", bucketID, "worst_count", len(clean))
}

func (s *OpenCodeService) ConsumePoseBucketRun(runID string) (string, []string, bool) {
	s.poseBucketRunsMu.Lock()
...
func (s *OpenCodeService) prunePoseBucketRunsLocked() {
...
func (s *OpenCodeService) MarkBroadcastRun(runID string) {
	s.broadcastRunsMu.Lock()
	s.broadcastRuns[runID] = true
	s.broadcastRunsMu.Unlock()
	slog.Info("broadcast run marked", "component", "opencode", "runID", runID)
}

func (s *OpenCodeService) ConsumeBroadcastRun(runID string) bool {
	s.broadcastRunsMu.Lock()
...
func (s *OpenCodeService) MarkWebChatRun(runID string) {
	s.webChatRunsMu.Lock()
	s.webChatRuns[runID] = true
	s.webChatRunsMu.Unlock()
	slog.Info("web chat run marked — TTS will be suppressed", "component", "opencode", "runID", runID)
}

func (s *OpenCodeService) IsWebChatRun(runID string) bool {
	s.webChatRunsMu.Lock()
...
func (s *OpenCodeService) ConsumeWebChatRun(runID string) bool {
	s.webChatRunsMu.Lock()
	ok := s.webChatRuns[runID]
	if ok {
...
func (s *OpenCodeService) MarkSilentRun(runID string) {
	s.silentRunsMu.Lock()
	s.silentRuns[runID] = true
	s.silentRunsMu.Unlock()
	slog.Info("silent run marked — TTS will be suppressed", "component", "opencode", "runID", runID)
}

func (s *OpenCodeService) IsSilentRun(runID string) bool {
	s.silentRunsMu.Lock()
...
func (s *OpenCodeService) ConsumeSilentRun(runID string) bool {
	s.silentRunsMu.Lock()
	ok := s.silentRuns[runID]
	if ok {
...
func (s *OpenCodeService) markTelegramRun(runID string, chatID string) {
	if runID == "" || chatID == "" {
		return
	}
	...
	slog.Info("telegram run marked — reply will be DMed", "component", "opencode", "runID", runID, "chatID", chatID)
}
...
func (s *OpenCodeService) hasTelegramRun(runID string) bool {
	s.telegramRunsMu.Lock()
...
func (s *OpenCodeService) consumeTelegramRun(runID string) string {
	s.telegramRunsMu.Lock()
...
func (s *OpenCodeService) markSlackRun(runID string, origin slackRun) {
	...
	slog.Info("slack run marked — reply will be posted to slack",
		"component", "opencode", "runID", runID, "channel", origin.channel)
}
...
func (s *OpenCodeService) hasSlackRun(runID string) bool {
	if runID == "" {
		return false
	}
	s.slackRunsMu.Lock()
...
func (s *OpenCodeService) markDiscordRun(runID string, channelID string) {
	if runID == "" || channelID == "" {
		return
	}
...
// still awaiting its reply — the typing keeper polls this to know when to stop.
func (s *OpenCodeService) hasDiscordRun(runID string) bool {
	if runID == "" {
		return false
	}
	s.discordRunsMu.Lock()
...
func (s *OpenCodeService) consumeDiscordRun(runID string) string {
	s.discordRunsMu.Lock()
...
func (s *OpenCodeService) consumeSlackRun(runID string) (slackRun, bool) {
	s.slackRunsMu.Lock()
...
func (s *OpenCodeService) pruneStalePendingChatLocked() {
...
func (s *OpenCodeService) HasFreshPendingChatSend() bool {
...
func (s *OpenCodeService) SetPendingChatTrace(runID string, message string) {
	s.pendingChatMu.Lock()
...
func (s *OpenCodeService) RemovePendingChatTraceByRunID(target string) bool {
	if target == "" {
		return false
	}
	s.pendingChatMu.Lock()
...
func (s *OpenCodeService) MatchPendingByMessage(needle string) string {
