...
func (s *OpenCodeService) StartHALVoice(deepgramKey, llmKey, sttKey, ttsKey, llmBaseURL, sttBaseURL, ttsBaseURL, ttsVoice, ttsInstructions, ttsProvider string) error {
	...
	slog.Info("HAL voice pipeline started", "component", "opencode")
...
func (s *OpenCodeService) SetVolume(pct int) error {
	if err := hal.SetVolume(pct); err != nil {
		return err
	}
	slog.Info("speaker volume set", "component", "opencode", "pct", pct)
	return nil
}

func (s *OpenCodeService) StopTTS() error {
	if err := hal.StopTTS(); err != nil {
		return err
	}
	if err := hal.StopAudio(); err != nil {
		slog.Warn("stop audio failed", "component", "opencode", "error", err)
	}
	slog.Info("speaker stopped (TTS + music)", "component", "opencode")
	return nil
}

func (s *OpenCodeService) SendToHALTTS(text string) error {
	...
	slog.Info("TTS sent", "component", "opencode", "text", truncRunes(text, 80))
	s.monitorBus.Push(domain.MonitorEvent{Type: "tts", Summary: text})
	return nil
}

func (s *OpenCodeService) SendToHALTTSQueue(text string) error {
	text = stripForTTS(text)
	if text == "" {
		return nil
	}
	...
	slog.Info("TTS queued", "component", "opencode", "text", truncRunes(text, 80))
	s.monitorBus.Push(domain.MonitorEvent{Type: "tts", Summary: text})
	return nil
}
