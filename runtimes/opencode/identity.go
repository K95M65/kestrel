...
// OpenCode's IDENTITY.md is copied 1-for-1 from OpenClaw (presync.sh §1 carries it
...
func identityPath() string {
	return filepath.Join(opencodeWorkspaceDir, "IDENTITY.md")
}

// WatchIdentity polls IDENTITY.md in the OpenCode workspace and pushes updated wake
// words to HAL whenever the agent's name changes (e.g. the user says "call yourself
// Noah"). Mirrors openclaw.WatchIdentity.
func (s *OpenCodeService) WatchIdentity(ctx context.Context) {
	path := identityPath()
	var lastName string
	for {
		select {
...
func (s *OpenCodeService) UpdateIdentityName(name string) error {
	...
	slog.Info("identity name updated", "component", "opencode", "name", name, "path", path)

	// Reflect the rename into the AGENTS.md persona inline block so the very
	// next `opencode run` turn already sees the new name. Best-effort — the block
