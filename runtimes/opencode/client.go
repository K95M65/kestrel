...
func (s *OpenCodeService) StartWS(ctx context.Context, handler domain.AgentEventHandler) {
	...
	for {
		...
		if err != nil {
			slog.Warn("websocket disconnected, reconnecting", "component", "opencode", "error", err, "backoff", reconnectBackoff)
			flow.Log("ws_disconnect", map[string]any{"error": err.Error(), "backoff_s": reconnectBackoff.Seconds()})
		} else {
			slog.Warn("websocket connection closed, reconnecting", "component", "opencode", "backoff", reconnectBackoff)
	...
	if s.wsHasConnected.Swap(true) {
		go func() {
			...
			if err := hal.SpeakCached(phrase); err != nil {
				slog.Warn("reconnect TTS failed", "component", "opencode", "error", err)
			}
		}()
	}
