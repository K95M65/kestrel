# OpenCode runtime — work log

Task: add OpenAI OpenCode CLI as the 4th agent runtime (openclaw / hermes / picoclaw / **opencode**),
mirroring the `feature/claude-code` branch's blueprint (Claude Code runtime by another dev).
Branch: `feat/opencode` (forked from `main` @ 118ff545, includes the ErrNotSupportedByRuntime
...
| Decision | Choice | Why |
|---|---|---|
| Transport | WS bridge on `ws://127.0.0.1:18793/opencode/ws/`, token `autonomous_opencode_token` | Mirror `feature/claude-code` (bridge.py + WS, picoclaw-shaped Go client) — team-established pattern |
| Bridge | **Go** (`runtimes/opencode/gatewayd`, compiled into os-server; unit runs `os-server opencode-gatewayd`) — user decision 2026-07-07 after weighing vs the claudecode branch's Python bridge: same-language repo, `go test` in CI, zero device deps (no python3/websockets), OTA rides os-server. Per-turn `opencode exec --json` subprocess (exec is the stable automation surface; `opencode app-server` is experimental/version-coupled). An earlier Python bridge.py existed briefly and was deleted. | |
| Session | `thread_id` from `thread.started`, persisted `/root/.opencode/session.json`, `opencode exec resume <id>`; `session.new` frame → fresh | History lives on disk under `$OPENCODE_HOME/sessions/` — process exit ≠ session loss |
| Auth (phase 1) | `OPENAI_API_KEY` = config.json `llm_api_key`, provider base_url = `llm_base_url` (campaign-api) | User decision: route through campaign-api. ChatGPT-subscription `opencode login --device-auth` deferred to phase 2 (share pairing plumbing with claudecode's `ClaudeLoginPairer` after that branch merges) |
| Wire API | `wire_api = "responses"` — OpenCode removed chat-completions (~2/2026) | ⚠️ VERIFY ON DEVICE: campaign-api must serve `{base}/responses` |
| CLI install | Pinned GitHub release `rust-v0.142.5`, `opencode-aarch64-unknown-linux-musl.tar.gz` → /usr/local/bin/opencode | picoclaw's pinned-binary pattern; musl static, no runtime deps |
...
| Instructions file | `AGENTS.md` in workspace — opencode reads it natively; OS-managed block reused from picoclaw's (openclaw-derived) | Zero-translation persona slot |
| Channels | **Telegram device-owned inbound** (2026-07-07): os-server runs the getUpdates receive loop itself (`telegram_poll.go`), started from StartWS so it exists only while opencode is the active runtime (no 409 poller conflicts — the hermes lesson). SupportedChannels=[telegram], AddChannel(telegram)=honest no-op success (creds in config.json drive the loop), slack/discord → ErrChannelNotSupported. UPDATE 2026-07-07: Slack inbound added via HTTP-mode proxy path (domain.SlackBridge in slack.go) → SupportedChannels=[telegram, slack]; discord/whatsapp still ErrChannelNotSupported. History: the original "telegram device-owned (picoclaw model)" copy was FALSE (picoclaw's inbound lives in its own binary) → corrected to none-inbound earlier on 2026-07-07 → real device-owned loop built the same day, closing TODO(opencode-telegram) | OpenCode CLI has no channel layer, so the OS owns the receive loop |
...
| MCP | `WriteMCPEntry` edits `/root/.opencode/config.toml [mcp_servers.<name>]` via go-toml/v2; presync regenerates the config head but preserves the `[mcp_servers` tail | opencode config.toml is the only MCP slot; supports streamable HTTP + headers |

## Research (verified against official docs 2026-07-07)

- `opencode exec "<prompt>" --json` → JSONL: `thread.started` (thread_id), `item.started/completed`
...
- `opencode exec resume <id>`, sessions at `$OPENCODE_HOME/sessions/YYYY/MM/DD/rollout-*.jsonl`.
- Images: `opencode exec -i <path>` (repeatable).
...
- Watch items: rollout JSONL grows unbounded (rotation policy on our side); pin CLI version per OTA;
  `opencode proto` is gone, `app-server` experimental — do not build on them.

## Progress

- [x] Research + fact-check OpenCode CLI (agent report in session; summary above)
- [x] `feat/opencode` branch from main; stale local claudecode files removed
- [x] **Bridge (Go)** `runtimes/opencode/gatewayd/` — WS server (gorilla), auth 4401, single-client
      replace, per-turn `opencode exec` (Setpgid group-kill, timeout), verbatim JSONL forwarding,
      session.json persistence, resume-retry-fresh, attachments via `-i`; 5 tests incl. `-race` PASS.
      Entry: `os-server opencode-gatewayd` subcommand (cmd/os-server/main.go)
- [x] `install.sh` — pinned binary download (rust-v0.142.5 musl), jq/curl only, opencode.service unit
      (`ExecStart=os-server opencode-gatewayd`), verify hook; bash -n clean
...
- [x] Go package `runtimes/opencode/` — full AgentGateway: translator.go (opencode exec JSONL → WSEvent),
      ...
      NewSession → session.new frame), reset (wipe /root/.opencode), runtime (version probe),
...
- [x] Glue: domain AgentRuntimeOpenCode + AgentRuntimes, factory case + transport map, version cache
      (handler_api_monitor + populate), logs.go journal:opencode.service mapping, web
      AgentRuntimeSection blurbs (+picoclaw blurb backfilled)
- [x] Docs: docs/agentic/opencode.md (243 lines) + docs/vi/agentic/opencode_vi.md + CLAUDE.md table row
- [x] go build ./... + full go test + `tsc -b` + GOOS=linux GOARCH=arm64 build — ALL GREEN
- [x] Persona migration adapter `migrate_persona/runtime_opencode.go` (OpenClaw-identical layout,
      rebrandToOpenCode + reOpenCode added to the other 3 adapters) → any-pair opencode↔openclaw/hermes/
      picoclaw incl. reverse; presync §1 remains the skills carrier
- [x] LLM config migration adapter `migrate_config/runtime_opencode.go` (read/write base_url in
      config.toml + OPENAI_API_KEY in .env; registered + OpenCodeHome in DefaultOptions) —
      key/baseURL carries across switches both directions, before presync even runs
- [x] MQTT switch path: `opencode.setup` kind (domain KindOpenCodeSetup + dispatcher case →
      handleRuntimeSetup) — web AND MQTT can now switch
- [x] Deep verification vs REAL opencode 0.142.5 binary + tag source (agent ran it):
      ...
      * CONFIRMED empirically: translator field names (item discriminator "type", usage names),
        ...
        names + binary name, `opencode --version` = "opencode-cli 0.142.5", env_key=OPENAI_API_KEY
...
- [x] Round-2 adversarial review (independent agent) + ALL 6 findings fixed:
      ...
      * HIGH: telegram was dead (picoclaw /root/.lumi leftover; NO device-owned inbound
        loop exists — picoclaw's lives inside its own binary) -> opencode is now HONEST:
        SupportedChannels=nil, ErrChannelNotSupported for all, outbound-only
        TelegramSender kept (explicit-ID DMs), TODO(opencode-telegram)
      ...
      * LOW: item.updated dedupe (ensureToolStart), install.sh exact-version guard,
        dead code removed (opencodeBin/envWithoutHome/picoStopVerifyTimeout), lying
...
- [x] Behavioral audit of sed-adapted files: skills pipeline verified coherent —
      ...
      AGENTS.md hint ALL resolve to /root/.opencode/skills (opencode's NATIVE $OPENCODE_HOME/skills
      discovery root — the `@` picker + native skill loading; NOT workspace/skills, which
      opencode never scans); markdown block changes no longer restart the gateway (opencode
...
- [x] campaign-api /responses probe (2026-07-07, from lamp-ac82 with the device key):
      ...
      /api/v1/responses) → **BLOCKER CONFIRMED**: opencode cannot chat through campaign-api
...
- [x] DEVICE-VERIFIED 2026-07-07 on lamp-ac82: opencode CLI installed manually, subscription
...
- [x] Device-owned Telegram inbound (`telegram_poll.go`, 2026-07-07): getUpdates long-poll
      ...
      advanced), offset persisted atomically to /root/.opencode/telegram_offset.json, accepted
      ...
      run/silent marking). TODO(opencode-telegram) closed. NOT device-verified.
...
- [x] Slack inbound, HTTP mode (2026-07-07): OpenCodeService implements domain.SlackBridge
      (slack.go, modeled on runtimes/hermes/slack.go), so the existing bff-proxy → MQTT
      slack_event dispatch (slack_event_handler.go type-assert at :151) routes events to
      opencode with zero server-side diff. Parse/allowlist (config.SlackUserID)/thread
      ...
      (no reply). SKIPPED vs hermes (intentional): progressive streaming (opencode has no
...
- [ ] Device verify: persona inline block in AGENTS.md (pre-fix, opencode introduced itself as "OpenCode" instead of the device persona name)
- [x] Device prepped for full switch test (2026-07-07): manual opencode binary REMOVED
      ...
      backend ships /responses; at that point DELETE /root/.opencode/auth.json and presync
      auto-flips back to api-key mode on the next boot.
- [ ] Phase 2: ChatGPT-subscription auth **pairing flow** (`opencode login --device-auth` UX) —
