...
// Channels are DEVICE-OWNED under OpenCode. The OpenCode CLI has no channel layer
// of its own (unlike picoclaw, whose runtime binary polls the Telegram Bot API
// itself — see runtimes/picoclaw/presync.sh), so os-server runs both inbound
// paths:
...
//     runs ONLY while opencode is the active runtime and can never fight another
...
//     events route here only while opencode is active. Replies go back via
