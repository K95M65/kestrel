...
func (t *TelegramSender) Send(msg string, imagePath string) error {
	...
	if len(targets) == 0 {
		slog.Warn("telegram: no chats found", "component", "picoclaw")
		return nil
	}

	slog.Info("telegram broadcast", "component", "picoclaw", "chats", len(targets), "hasImage", imagePath != "")
	...
	if imagePath != "" {
		if data, err := os.ReadFile(imagePath); err == nil {
			...
			slog.Warn("telegram: failed to read image", "component", "picoclaw", "path", imagePath, "err", err)
		}
	}
...
func (t *TelegramSender) SendToUser(telegramID string, msg string, imagePath string) error {
	...
	slog.Info("telegram dm", "component", "picoclaw", "telegram_id", telegramID, "hasImage", imagePath != "")
...
func (t *TelegramSender) SendToUserWithMedia(telegramID string, msg string, imagePaths []string) error {
	...
	for i, p := range imagePaths {
		...
		if err != nil {
			slog.Warn("telegram: media group skip", "component", "picoclaw", "path", p, "err", err)
			continue
		}
	...
	switch len(photos) {
	case 0:
		slog.Warn("telegram: no readable media, falling back to text", "component", "picoclaw", "telegram_id", telegramID)
		client := &http.Client{Timeout: 10 * time.Second}
		sendTelegramMessage(client, botToken, telegramID, msg)
		return nil
	case 1:
	...
	slog.Info("telegram media group", "component", "picoclaw", "telegram_id", telegramID, "count", len(photos))
	...
	if err != nil {
		slog.Error("telegram sendMediaGroup failed", "component", "picoclaw", "chatID", telegramID, "err", err)
		return err
	}
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	if resp.StatusCode != 200 {
		slog.Error("telegram sendMediaGroup error", "component", "picoclaw", "chatID", telegramID, "status", resp.StatusCode, "body", string(body))
		return fmt.Errorf("sendMediaGroup status %d", resp.StatusCode)
	}
	slog.Info("telegram sendMediaGroup sent", "component", "picoclaw", "chatID", telegramID, "photos", len(photos))
...
func sendTelegramMessage(client *http.Client, token, chatID, text string) {
	...
	if err != nil {
		slog.Error("telegram sendMessage failed", "component", "picoclaw", "chatID", chatID, "err", err)
		return
	}
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	if resp.StatusCode != 200 {
		slog.Error("telegram sendMessage error", "component", "picoclaw", "chatID", chatID, "status", resp.StatusCode, "body", string(body))
		return
	}
	slog.Info("telegram sendMessage sent", "component", "picoclaw", "chatID", chatID)
}
...
func sendTelegramPhoto(client *http.Client, token, chatID, caption string, photo []byte) {
	...
	resp, err := client.Post(apiURL, w.FormDataContentType(), &buf)
	if err != nil {
		slog.Error("telegram sendPhoto failed", "component", "picoclaw", "chatID", chatID, "err", err)
		return
	}
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	if resp.StatusCode != 200 {
		slog.Error("telegram sendPhoto error", "component", "picoclaw", "chatID", chatID, "status", resp.StatusCode, "body", string(body))
		return
	}
	slog.Info("telegram sendPhoto sent", "component", "picoclaw", "chatID", chatID)
}
