...
Ported verbatim from perception-service. Uses the torch ``silero-vad`` package
(torch is already present on-device via lerobot/ultralytics). This is the gate:
if it raises PreprocessRejected, HAL treats the clip as audio-level rejected and
never sends it to the embedding server.
"""
...
DEFAULT_MIN_VOICE_RATIO: float = 0.4
